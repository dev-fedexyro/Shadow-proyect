...
global.botNumber = global.opts['mobile'] || global.opts['n'] || ''
  ...
  if (global.db) setInterval(async () => {
    ...
    if (opts['autocleartmp']) {
      ...
      tmp.forEach((filename) => {
        try {
          execSync(`find ${filename} -amin 3 -type f -delete`, { stdio: 'pipe' })
        } catch (e) {
        }
      })
    }
...
async function connectionUpdate(update) {
  ...
  if ((update.qr != 0 && update.qr != undefined) && (opcion === '1' || methodCodeQR)) {
  ...
  if (connection === "open") {
    ...
    console.log(chalk.green.bold(`[ ✿ ]  Conectado como: ${userName} (${userJid})`))
  }
  
  let reason = new Boom(lastDisconnect?.error)?.output?.statusCode
  if (connection === "close") {
    if ([401, 440, 428, 405].includes(reason)) {
      console.log(chalk.red(`→ (${code}) › Sesión cerrada. Borra la carpeta ${global.sessions} para volver a conectar.`))
    }
...
async function filesInit() {
  for (const filename of readdirSync(pluginFolder).filter(pluginFilter)) {
    try {
      ...
      conn.logger.error(`Error al cargar plugin '${filename}': ${e}`)
